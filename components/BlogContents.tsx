    ...
    <section className="page-section writing-page" aria-labelledby="writing-page-title">
      ...
      <div className="article-list">{posts.map((post, index) => (
        <article key={post.slug} className="article-card">
          <div className="article-image"><Image src={post.imageUrl} alt="" fill preload={index === 0} quality={80} sizes="(max-width: 620px) calc(100vw - 2rem), (max-width: 1200px) calc(50vw - 2rem), 560px" /></div>
          <div className="article-card-copy"><div className="article-meta"><time dateTime={post.publishDate}>{new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' }).format(new Date(post.publishDate))}</time><span>{post.categories[0]}</span></div><h2>{post.title}</h2><p>{post.description}</p><Link className="text-link article-read-link" href={`/blog/${post.slug}`}><span>Read article</span><ExternalLinkIcon /></Link></div>
        </article>
      ))}</div>
    </section>
  );
