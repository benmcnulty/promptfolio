...
Release remains a separate, explicit action. Before pushing the private `main`, confirm the intended commit, required GitHub checks, Vercel project/root settings, environment variables, and the previously deployed revision. Record the current production deployment as the rollback target. After an authorized deployment, smoke-test every primary route, catalog filtering and search, theme persistence, the three APIs, referenced images, security headers, and the apex-to-`www` redirect.

The case study at `/blog/rebuilding-promptfolio-without-losing-its-spark` intentionally uses matched production `before` captures and release-build `after` captures. If design review changes the interface, recapture every `after` image at the same viewport before the final build. That capture pass also emits the PNG source used to generate Open Graph and X/Twitter images, keeping the article, metadata previews, and shipped interface aligned. Following deployment, verify the article metadata and comparison dialogs against production.
