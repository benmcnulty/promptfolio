...
Validated locally on September 13, 2026 as the release candidate integrated into local `main`. The original checkout was `9d17a749d37b59c6348c2a61e622ee3b513cb7f1`.

## Automated gates

- ESLint flat configuration, TypeScript, Jest, and the Next.js production build pass.
- Jest runs 17 tests across four suites. Maintained application logic reports 100% statements, lines, and functions and 97.36% branches; filtering/query logic and API transformations have 100% branch coverage.
- Playwright runs 297 active checks across desktop Chromium, Firefox, and WebKit plus mobile Chromium and mobile WebKit; 103 opt-in visual-baseline or platform-specific cases are skipped in the default local run. The final local pass ran each browser profile separately against the same production build to avoid cumulative process exhaustion under the machine's nonstandard Node 26 runtime. Coverage includes every primary, utility, error, and article route; internal links; APIs; URL-driven filters/search; animated catalog updates; adaptive list/grid presentation; category-tinted list surfaces; labeled compact rows with reversible keyboard-accessible expansion; stable scroll position while switching expanded entries; continuous expanded-card gradients; responsive page-ending rhythm; responsive optimized image requests; the three-state theme cycle and persistence; browser-chrome theme color; skip navigation; article callouts and comparisons; category and brand iconography; generated social images; dialog keyboard behavior; time-service failure; 404 recovery and metadata; accessibility; and reflow from 320-by-568 phone portrait through 844-by-390 phone landscape, tablet, laptop, and 1920-by-1080 desktop geometry.
...
- Visual coverage defines light and dark full-page captures for all 16 public pages at desktop and mobile sizes, plus theme, filter, carousel, and dialog states. Candidate baselines are generated in the fixed Ubuntu CI environment by manual workflow dispatch and uploaded for review. The case study's four final `after` images were regenerated from the release build and inspected at their intended 1440-by-800 desktop and 390-by-780 mobile viewports. The metadata routes now compose Open Graph and X/Twitter cards from that same current homepage capture during the build.
...
The local machine provided Node 26.5.0 and npm 12.0.2, so the standardized Node 22/npm 10.9.9 execution remains a required GitHub Actions pull-request gate before deployment. ESLint 9 is pinned because the installed Next.js lint stack does not yet support ESLint 10; npm reports the pin as outside ESLint's current support window, though the production audit is clean. Production deployment revision alignment, Vercel settings, rollback readiness, live security headers, external GPT destinations, manual screen-reader behavior, and production smoke tests remain release-time checks. The redesign is integrated into the private repository's local `main`; it has not been pushed or deployed.
